      ...
      Begin VB.Frame fr1T 
         ...
         Begin VB.Frame fr1Det 
            ...
            Begin MSForms.ComboBox cboDesig 
               ...
               FontPitchAndFamily=   2
            End
            Begin MSForms.ComboBox cboGrade 
               Height          =   315
               Left            =   1170
               TabIndex        =   110
               Top             =   990
         ...
         Begin VB.Frame fr1Iden 
            ...
            Begin VB.Frame fraMachine 
               ...
               End
            End
            Begin MSForms.ComboBox cboReporting 
               Height          =   315
               Left            =   6570
               TabIndex        =   112
               Top             =   1305
            ...
            Begin MSForms.ComboBox cboEnt 
               ...
               FontPitchAndFamily=   2
            End
            Begin MSForms.ComboBox cboTravel 
               Height          =   315
               Left            =   6555
               TabIndex        =   35
               Top             =   240
...
'' The Code for Displaying the Records is bit Complicated
'' What Happens is that the Selection Criteria is Distributed in Three different places
'' 1. Grid Selection   2.Code Combo Selection   3.Name Combo Selection
'' At 2 & 3 the Record is Found it is Only Displayed it it does not Match with the Record
'' Already Displayed.
'' At 1 The Record is Selected and Displayed Only When it Does not Match with the Record
'' Already.
'' The Display Part is Called at Three different Places i.e In Code Click,Combo Click and
'' the Tab Click
'' To keep Track From where the Selection is Coming Boolean Variable blnCodeMatch is Used.
'' These Complications Come because Assigning the Tab Throgh Code also Calls the Tab Click
'' Event.
'' Also When Other Tabs i.e Personal Details & Other Details are Clicked Record is only
'' Displayed when there is no Record to Display and the record is Selected from the Grid.
'' Also During the Set Focus the Tab Which has got the Active Control is Made Visible
'' if it is not already Visible. for the bytmode is Temporarily Added to 10 and as Soon as
'' the Tab is Displayed it is Reduced back by 10.
...
Private Sub cboCORule_Change()
'APOORVA TO MAKE SURE THAT ONLY CO/OT IS SELECTED
''COMMENTED SG07
''If cboCORule.Text <> "None" Then
''    cboOTRule.Enabled = False
''ElseIf cboCORule.Text = "None" Then
''    cboOTRule.Enabled = True
''End If
End Sub
...
Private Sub cboOTRule_Change()
''COMMENTED SG07
''If cboOTRule.Text <> "None" Then
''   cboCORule.Enabled = False
''ElseIf cboOTRule.Text = "None" Then
''    cboCORule.Enabled = True
''End If
End Sub
...
If GetFlagStatus("MICROGOA") Then
lblGrade.Caption = "Contractor"
End If
If GetFlagStatus("SAPCODE") Then
lblSGroup.Caption = "SAPCODE"
End If
If GetFlagStatus("BANCO1") Then
lblReporting.Caption = "Section"
lblReporting.Visible = True
cboReporting.Visible = True
End If
If GetFlagStatus("DESIGNATION") Then
...
Call FillCombos                             '' Fills the Necessary Combos
Call OpenMasterTable                        '' Opens the Employee Master Table
strSort = "Empcode"
...
'this AlphaNumeicEmpCode add by jagdish
...
adrsC.Open "Select * From NewCaptions Where ID Like '23%'", VstarDataEnv.cnDJConn, adOpenStatic
...
lblTravel.Caption = NewCaptionTxt("23019", adrsC)           '' Travel By
'' Schedule Frame
...
'' Travel Combo
cboTravel.AddItem "Bus"
cboTravel.AddItem "Other"
'' Sex Combo
cboSex.AddItem "M"
cboSex.AddItem "F"
Exit Sub
...
VstarDataEnv.cnDJConn, adOpenKeyset, adLockOptimistic       '' Fill Name Combo
If Not (adrsDept1.BOF And adrsDept1.EOF) Then
    Do While Not adrsDept1.EOF
    ...
    Loop
End If
If GetFlagStatus("WEBMIS") Then     'Girish 13-06-2009
    Call ComboFill(cboReporting, 1, 2)
End If
...
'Girish 18-04-09
'added by prashant for subros(pune)
''Grade Combo
If GetFlagStatus("ManPower") Or GetFlagStatus("IOC") = True Then
    Call ComboFill(cboGrade, 4, 2)
End If
If GetFlagStatus("WEBMIS") Then     'Girish 10-06-2009
    Call ComboFill(cboReporting, 1, 2)
End If
If GetFlagStatus("BANCO1") Then     'Girish 10-06-2009
    Call ComboFill(cboReporting, 18, 2)
End If
If GetFlagStatus("MACHINE") Then Call ComboFill(cboMachine, 19, 2)
If GetFlagStatus("DESIGNATION") Then Call ComboFill(cboDesig, 20, 2)
Exit Sub
ERR_P:
End Sub
...
End Select
If Not GetFlagStatus("ExportToSQL") Then
    optWrk.Visible = False
    optStf.Visible = False
End If
txtCode.MaxLength = pVStar.CodeSize
txtCard.MaxLength = pVStar.CardSize
'Girish 29-04-09
If GetFlagStatus("ManPower") Or GetFlagStatus("IOC") Then
...
End If
If GetFlagStatus("WEBMIS") Then     'Girish 11-06-2009
    cboReporting.Visible = True
    lblReporting.Visible = True
End If
fraMachine.Visible = GetFlagStatus("MACHINE")
End Sub
...
End If
If GetFlagStatus("LocationRights") Then strTmp = strCurrData
adrsEmp.Open "Select Empmst.* from EmpMst " & strTmp & " Order by EmpCode,Name", _
...
adrsEmp.Requery               '' Requeries the Recordset for any Updated Values
...
If Not (adrsEmp.EOF) Then
    ...
    txtDesc.Text = IIf(IsNull(adrsEmp("Designatn")), "", adrsEmp("Designatn"))
    txtName2.Text = IIf(IsNull(adrsEmp("Name2")), "", adrsEmp("Name2"))
    
    If GetFlagStatus("ExportToSQL") Then
        If adrsEmp("STFWRK") = "S" Then
            optStf.Value = True
        Else
            optWrk.Value = True
        End If
    ...
    Call PutComboValues
    cboTravel.ListIndex = IIf(adrsEmp("Conv") = "B", 0, 1)
    '' Define Schedule
    ...
    'add by jgadish
    If GetFlagStatus("FirstWeekOffOfMonth") Then
        If adrsWOTGL.State = 1 Then adrsWOTGL.Close
        adrsWOTGL.Open "SELECT * FROM MonthTGL WHERE empcode='" & Shft.Empcode & "'", VstarDataEnv.cnDJConn
        If Not (adrsWOTGL.EOF And adrsWOTGL.BOF) Then
            Shft.WO4 = FilterNull(adrsWOTGL.Fields("days"))
        End If
    End If
    ''
    'add by jagdish for Robosoft
    If GetFlagStatus("WO") Then
        Shft.WORule = FilterNull(adrsEmp.Fields("WeekOffRule"))
    End If
    ''
    ...
    If GetFlagStatus("IOC") Then
    txtmedical.Text = IIf(IsNull(adrsEmp("MedicalDate")), "", adrsEmp("MedicalDate"))
    End If
    '' Tab 3
    '' Details Frame
    ...
    'Girish 29-04-09
    If GetFlagStatus("ManPower") Or GetFlagStatus("IOC") Then cboGrade.Text = IIf(IsNull(adrsEmp!gradecode), "", adrsEmp!gradecode)
    If GetFlagStatus("MICROGOA") Then cboGrade.Text = IIf(IsNull(adrsEmp!Contractorcode), "", adrsEmp!Contractorcode)
    If GetFlagStatus("WEBMIS") Then     'Girish 11-06-2009
        cboReporting.ListIndex = -1
        cboReporting.Text = IIf(IsNull(adrsEmp("ReporteesName")), adrsEmp!Empcode, adrsEmp!ReporteesName)
    End If
End If
Exit Sub
ERR_P:
...
Dim strTmp As String
strTmp = "Categoty"
cboCat.Text = adrsEmp("Cat")            '' Category
strTmp = "Department"
cbodept.Text = adrsEmp("Dept")          '' Department
strTmp = "Group"
cboGroup.Text = IIf(IsNull(adrsEmp("Group")), "0", adrsEmp("Group"))        '' Group
strTmp = "Location"
cboLoca.Text = IIf(IsNull(adrsEmp("Location")), "0", adrsEmp("Location"))  '' Location
strTmp = "Company"
cboCompany.Value = IIf(IsNull(adrsEmp("Company")), "", adrsEmp("Company"))  '' Company
strTmp = "Division"
cboDiv.Value = IIf(IsNull(adrsEmp("Div")), "", adrsEmp("Div"))  '' Division
strTmp = "OT Rule"
cboOTRule.Value = IIf(IsNull(adrsEmp("OTCode")) Or adrsEmp("OTCode") = 100, "None", _
adrsEmp("OTCode")) '' OT Rule
strTmp = "CO Rule"
cboCORule.Value = IIf(IsNull(adrsEmp("COCode")) Or adrsEmp("COCode") = 100, "None", _
adrsEmp("COCode")) '' CO Rule
'Girish 18-04-09
''added by prashant
If GetFlagStatus("Manpower") Or GetFlagStatus("IOC") = True Then
cboGrade.Value = IIf(IsNull(adrsEmp("Gradecode")), "None", adrsEmp("Gradecode"))     ''Grade
End If
If GetFlagStatus("MICROGOA") = True Then
cboGrade.Value = IIf(IsNull(adrsEmp("Contractorcode")), "None", adrsEmp("Contractorcode"))     ''Grade
End If
If GetFlagStatus("MACHINE") = True Then cboMachine.Value = IIf(IsNull(adrsEmp("MachineCode")), "", adrsEmp("MachineCode"))
If GetFlagStatus("DESIGNATION") = True Then cboDesig.Value = IIf(IsNull(adrsEmp("Designatn")) Or adrsEmp("Designatn") = "", "", adrsEmp("Designatn"))
Exit Sub
ERR_P:
...
Select Case bytMode
    ...
    Case 3
        ...
        End If
        'add by jagdish
        If GetFlagStatus("EditLeftDate") Then
            If Not FilterNull(adrsEmp("LeavDate")) = "" Then
                MsgBox "Left employee not eligible to Edit", vbExclamation
                Exit Sub
            End If
        End If
        Call EditAction
...
If GetFlagStatus("WEBMIS") Then
    cboReporting.ListIndex = -1
End If
If GetFlagStatus("MACHINE") Then cboMachine.Value = ""
If GetFlagStatus("DESIGNATION") Then cboDesig.Value = ""
End Sub
...
If MSF1.Rows > 1 Then
    ...
    If Not adrsEmp.EOF Then
        ''SG07
        ''MsgBox NewCaptionTxt("23054", adrsC), vbExclamation
    ...
    End If
End If
'' Check for Permission Card Numbers
...
If Trim(txtLeft.Text) <> "" Then
    ...
    End If
End If
'' Check for Shift Date Greater then Leave Date
If Trim(txtLeft.Text) <> "" Then
    ...
    End If
End If
'' Check for Birth Date Greater than Joindate
...
If Trim(txtConf.Text) <> "" Then
    ...
    End If
End If
'add by jagdish for division code validation
...
If GetFlagStatus("WEBMIS") Then     'Girish 13-06-09
    If cboReporting.MatchFound = False And cboReporting.Text <> "" Then
        MsgBox "Please Select The Proper Reportees Person", vbExclamation, "Error ......"
        cboReporting.SetFocus
        ValidateAddmaster = False
        Exit Function
    End If
End If
If GetFlagStatus("DESIGNATION") Then
    If Trim(cboDesig.Text) = "" Then
        MsgBox "Please Select Employee Designation", vbExclamation
        cboDesig.SetFocus
        ValidateAddmaster = False
        Exit Function
    End If
End If
''SG07
''If cboOTRule.Enabled = True And cboCORule.Enabled = True Then           'rauf
''    MsgBox "U Can Choose only one Option !!! CoRule Or OTRule At A Time ", vbOKCancel, Me.Caption
'Else
'    cboOTRule.SetFocus
''End If
''
...
adRsInstall.Open "select * from Install", VstarDataEnv.cnDJConn
...
If Trim(txtLeft.Text) = "" Then
    strTmp(1) = "NULL"
Else
    strTmp(1) = GetTO_Char(txtLeft.Text)
End If
If Trim(txtDOB.Text) = "" Then
    strTmp(2) = "NULL"
Else
    strTmp(2) = GetTO_Char(txtDOB.Text)
End If
If Trim(txtConf.Text) = "" Then
    strTmp(3) = "NULL"
Else
    strTmp(3) = GetTO_Char(txtConf.Text)
End If
strTmp(4) = GetTO_Char(txtDOJ.Text)
''End
If Trim(txtSal.Text) = "" Then txtSal.Text = "0"
'' Insert Query
''SG07
Dim strFld As String, strFldValue
strFld = "": strFldValue = ""
If GetFlagStatus("MACHINE") Then
    strFld = ",MachineCode"
    strFldValue = ",'" & cboMachine.Text & "'"
End If
VstarDataEnv.cnDJConn.Execute "insert into EmpMst(EmpCode,Card,Name,Designatn,Entry,OTCode," & _
...
"Name2,WOHLAction,Action3Shift,AutoForPunch,ActionBlank,AutoG" & strFld & ") Values('" & txtCode.Text & "','" & _
txtCard.Text & "','" & Trim(txtName.Text) & "','" & IIf(GetFlagStatus("DESIGNATION"), Trim(cboDesig.Text), Trim(txtDesc.Text)) & "'," & cboEnt.Text & _
"," & IIf(cboOTRule.Text = "None", 100, cboOTRule.Text) & "," & _
IIf(cboCORule.Text = "None", 100, cboCORule.Text) & "," & IIf(chkAuto.Value = 0, 0, 1) & _
",'" & cboCat.Text & "'," & cbodept.Text & "," & cboGroup.Text & "," & cboLoca.Text & _
"," & cboCompany.Value & "," & cboDiv.Value & ",'" & IIf(cboTravel.Text = "Bus", "B", "O") & _
"'," & strTmp(0) & "," & strTmp(1) & "," & strTmp(2) & ",'" & txtBlood.Text & "'," & strTmp(4) & _
"," & strTmp(3) & ",'" & cboSex.Text & "','" & txtFreeF.Text & "','" & txtEmail.Text & "'," & _
txtSal.Text & ",'" & txtRef.Text & "','" & txtAdd1.Text & "','" & txtAdd2.Text & "','" & txtCity.Text & _
"','" & txtPin.Text & "','" & txtPhone.Text & "','" & txtHouse.Text & "','" & txtArea.Text & "','" & _
txtVill.Text & "','" & txtRoad.Text & "','" & txtDist.Text & "','" & txtState.Text & "','" & _
txtTel.Text & "','" & txtNat.Text & "','" & txtSpl(0).Text & "','" & txtSpl(1).Text & _
"','" & Trim(txtName2.Text) & "'," & Shft.WOHLAction & ",'" & Shft.Action3Shift & "'," & _
IIf(Shft.AutoOnPunch, 1, 0) & ",'" & Shft.ActionBlank & "','" & txtAutoG.Text & "'" & strFldValue & ")"
'ADD BY jagdish For TGL
If GetFlagStatus("WEBMIS") Then 'Girish 10-06-09
    VstarDataEnv.cnDJConn.Execute "Update Empmst Set ReporteesName =  '" & cboReporting & "' WHERE empcode='" & txtCode.Text & "'"
End If
If GetFlagStatus("FirstWeekOffOfMonth") Then
    VstarDataEnv.cnDJConn.Execute ("INSERT INTO MonthTGL VALUES('" & txtCode.Text & "','" & Shft.WO4 & "')")
End If
'ADD BY jagdish For Robosoft
If GetFlagStatus("WO") Then
    VstarDataEnv.cnDJConn.Execute "UPDATE Empmst SET WeekOffRule ='" & _
        Shft.WORule & "' WHERE empcode='" & txtCode.Text & "'"
End If
'Added by SG for Atlas
If GetFlagStatus("ExportToSQL") Then
    VstarDataEnv.cnDJConn.Execute "UPDATE Empmst SET STFWRK ='" & _
        IIf(optStf.Value = True, "S", "W") & "' WHERE empcode='" & txtCode.Text & "'"
End If

'Girish 29-04-09
If GetFlagStatus("ManPower") Or GetFlagStatus("IOC") Then
    VstarDataEnv.cnDJConn.Execute "Update Empmst Set GradeCode = " & cboGrade.Text & " Where Empcode = '" & txtCode.Text & "'"
End If
If GetFlagStatus("MICROGOA") Then
    VstarDataEnv.cnDJConn.Execute "Update Empmst Set Contractorcode = " & cboGrade.Text & " Where Empcode = '" & txtCode.Text & "'"
End If
SaveAddMaster = True
Exit Function
ERR_P:
...
Else
    ...
    = vbYes Then                                '' Delete the Record
        VstarDataEnv.cnDJConn.Execute "delete from EmpMst where EmpCode='" & txtCode.Text _
        & "'"
        Call AddActivityLog(lgDelete_Action, 1, 14)     '' Delete Log
        Call AuditInfo("DELETE", Me.Caption, "Deleted Employee: " & txtCode.Text)
        
        VstarDataEnv.cnDJConn.Execute "delete from lvbal" & Right(pVStar.YearSel, 2) & " where EmpCode='" & txtCode.Text & "'"
        VstarDataEnv.cnDJConn.Execute "delete from EmpMst where EmpCode='" & txtCode.Text & "'"
        If GetFlagStatus("WEBMIS") Then         'Girish     10-06-09
            VstarDataEnv.cnDJConn.Execute "delete from LeaveBal where EmpCode='" & txtCode.Text & "'"
        End If
...
ERR_P:
    If Err.Number = -2147467259 Or Err.Number = -2147217900 Then
            ...
            If (MsgBox("Do you want to Delete the Employee?", vbYesNo)) = vbYes Then
                ...
                If GetFlagStatus("WEBMIS") Then         'Girish     10-06-09
                    VstarDataEnv.cnDJConn.Execute "delete from LeaveBal where EmpCode='" & txtCode.Text & "'"
             '       VstarDataEnv.cnDJConn.Execute "delete from LeaveInfo where EmpCode='" & txtCode.Text & "'"
                End If
            End If
            Call FillEmpCodeName                        '' Reflect the ComboBoxes
            Call FillGrid                               '' Reflect the Grid
...
If MSF1.Rows > 1 Then
    ...
    adrsEmp.Find "Card='" & txtCard.Text & "'"
    If Not adrsEmp.EOF Then
        If txtCode.Text <> adrsEmp("EmpCode") Then
            MsgBox NewCaptionTxt("23054", adrsC), vbExclamation
            txtCard.SetFocus
            ValidateModMaster = False
            Exit Function
...
If Trim(txtConf.Text) <> "" Then
    ...
    End If
End If
'' if Everything is Validated Find the
adrsEmp.MoveFirst
adrsEmp.Find "EmpCode='" & txtCode.Text & "'"
...
If Trim(cboDiv.Value) = "" Then
        ...
        Exit Function
End If
If GetFlagStatus("ManPower") Or GetFlagStatus("IOC") And Trim(cboGrade.Value) = "" Then
        MsgBox "Grade should not blank", vbExclamation
        cboGrade.SetFocus
        ValidateModMaster = False
        Exit Function
End If
...
Dim strTmp(4) As String
If Shft.ShiftType = "F" Then
    strTmp(0) = "Styp='F',F_Shf='" & Shft.ShiftCode & "',SCode='100',Shf_Date=" & strDTEnc & _
    DateCompStr(Shft.startdate) & strDTEnc & "," & strKOff & " ='" & Shft.WO & "',Off2='" & Shft.WO1 & _
    "',WO_1_3='" & Shft.WO2 & "',WO_2_4='" & Shft.WO3 & "'"
Else
    strTmp(0) = "Styp='R',F_Shf='100',SCode='" & Shft.ShiftCode & "',Shf_Date=" & strDTEnc & _
    DateCompStr(Shft.startdate) & strDTEnc & "," & strKOff & " ='" & Shft.WO & "',Off2='" & Shft.WO1 & _
    "',WO_1_3='" & Shft.WO2 & "',WO_2_4='" & Shft.WO3 & "'"
End If
''For Mauritius 05-08-2003
''Start
If Trim(txtLeft.Text) = "" Then
    strTmp(1) = "LeavDate=NULL"
Else
    strTmp(1) = "LeavDate=" & GetTO_Char(txtLeft.Text)
End If
If Trim(txtDOB.Text) = "" Then
    strTmp(2) = "Birth_Dt=NULL"
Else
    strTmp(2) = "Birth_Dt= " & GetTO_Char(txtDOB.Text)
End If
If Trim(txtConf.Text) = "" Then
    strTmp(3) = "Confmdt=NULL"
Else
    strTmp(3) = "Confmdt=" & GetTO_Char(txtConf.Text)
End If
strTmp(4) = " Joindate=" & GetTO_Char(txtDOJ.Text)
''End
If Trim(txtSal.Text) = "" Then txtSal.Text = "0"
If Trim(txtAutoG.Text) = "" Then txtAutoG.Text = " "
''SG07
Dim strFld As String
strFld = ""
If GetFlagStatus("MACHINE") Then
    If Trim(cboMachine.Text) <> "" Then strFld = ",MachineCode='" & cboMachine.Text & "'"
End If
VstarDataEnv.cnDJConn.Execute "Update EmpMst Set Card='" & txtCard.Text & "',Name='" & _
txtName.Text & "',Designatn='" & IIf(GetFlagStatus("DESIGNATION"), cboDesig.Text, txtDesc.Text) & "',Entry=" & cboEnt.Text & ",OTCode=" & _
IIf(cboOTRule.Text = "None", 100, cboOTRule.Text) & _
",COCode=" & IIf(cboCORule.Text = "None", 100, cboCORule.Text) & ",Shf_Chg=" & _
IIf(chkAuto.Value = 0, 0, 1) & ",Cat='" & cboCat.Text & "',Dept=" & cbodept.Text & _
"," & strKGroup & " =" & cboGroup.Text & ",Location=" & cboLoca.Text & ",Company=" & _
cboCompany.Value & ",Div=" & cboDiv.Value & ", Conv='" & _
IIf(cboTravel.Text = "Bus", "B", "O") & "'," & strTmp(0) & "," & strTmp(1) & "," & _
strTmp(2) & ",BG='" & txtBlood.Text & "'," & strTmp(4) & _
"," & strTmp(3) & ",Sex='" & cboSex.Text & "',Qualf='" & txtFreeF.Text & "',Email_Id='" & _
txtEmail.Text & "',Salary=" & txtSal.Text & ",Reference='" & txtRef.Text & "',ResAdd1='" & _
txtAdd1.Text & "',ResAdd2='" & txtAdd2.Text & "',City='" & txtCity.Text & "',Pin='" & txtPin.Text & _
"',Phone='" & txtPhone.Text & "',Udf1='" & txtHouse.Text & "',Udf2='" & txtArea.Text & "',UDF3='" & _
txtVill.Text & "',UDF4='" & txtRoad.Text & "',UDF5='" & txtDist.Text & "',UDF6='" & txtState.Text & _
"',UDF7='" & txtTel.Text & "',UDF8='" & txtNat.Text & "',UDF9='" & txtSpl(0).Text & "',UDF10='" & _
txtSpl(1).Text & "',Name2='" & Trim(txtName2.Text) & "',WOHLAction=" & Shft.WOHLAction & _
",Action3Shift='" & Shft.Action3Shift & "',AutoForPunch=" & IIf(Shft.AutoOnPunch, 1, 0) & _
",ActionBlank='" & Shft.ActionBlank & "',AutoG='" & txtAutoG.Text & "'" & strFld & " Where EmpCode='" & txtCode.Text & "'"
...
If GetFlagStatus("FirstWeekOffOfMonth") Then
    ...
    adrsT.Open "SELECT * FROM MonthTGL WHERE empcode='" & txtCode.Text & "'", VstarDataEnv.cnDJConn
    'If Not adrsT.EOF Then adrsT.Find "EmpCode='" & txtCode.Text & "'"
    If adrsT.EOF Then
        VstarDataEnv.cnDJConn.Execute ("INSERT INTO MonthTGL VALUES('" & _
        txtCode.Text & "','" & Shft.WO4 & "')")
    Else
        VstarDataEnv.cnDJConn.Execute "UPDATE MonthTGL SET days='" & _
        Shft.WO4 & "' WHERE empcode='" & txtCode.Text & "'"
    End If
End If
''
'ADD BY jagdish For Robosoft
...
Call AuditInfo("EDIT", Me.Caption, "Edited Employee: " & txtCode.Text)
Exit Sub
ERR_P:
    ShowError ("Log Error :: " & Me.Caption)
End Sub
...
If KeyCode = vbKeyF10 Then Call ShowF10("23")
