...
Begin VB.Form frmOpening 
   ...
   Begin TabDlg.SSTab TB1 
      ...
      EndProperty
      TabCaption(0)   =   "List"
      TabPicture(0)   =   "frmOpening.frx":0000
      Tab(0).ControlEnabled=   0   'False
      Tab(0).Control(0)=   "MSF1"
      Tab(0).Control(0).Enabled=   0   'False
      Tab(0).ControlCount=   1
      TabCaption(1)   =   " "
      TabPicture(1)   =   "frmOpening.frx":001C
...
adrsC.Open "Select * from NewCaptions Where ID Like '07%'or ID Like '37%'", _
ConMain, adOpenStatic
Me.Caption = NewCaptionTxt("37001", adrsC)              '' Form Caption
...
lblFrom.Caption = NewCaptionTxt("37002", adrsC)         '' Encash on
...
Private Sub FillGrid()      '' Fills the Grid with the Leaves the Employee Has Added as
On Error GoTo ERR_P         '' Opening Balance
Dim bytCnt As Byte
If adrsDept1.State = 1 Then adrsDept1.Close
adrsDept1.Open "Select LCode,FromDate,ToDate,Days  From " & "LvInfo" & _
Right(pVStar.YearSel, 2) & " where Trcd= 1" & " and Empcode=" & "'" & _
    ...
    For intTmp = 0 To adrsDept1.Fields.Count - 1
        If UCase(adrsDept1(intTmp).name) <> "EMPCODE" Then  '' if Field is not Employee Code
            ...
            If Not (adrsRits.EOF And adrsRits.BOF) Then         '' if Leave Desc is Found
                ...
                bytTmp = bytTmp + 1
                If SubLeaveFlag = 1 And (UCase(adrsDept1(intTmp).name) = "HP" Or UCase(adrsDept1(intTmp).name) = "CM" Or UCase(adrsDept1(intTmp).name) = "EL") Then bytTmp = bytTmp - 1 ' 15-10
            End If
        End If
    Next
...
Call GetCat
If bytMode <> 4 Then
    '' Get All the Leaves the Employee has Opening Balance that Year
    bytMode = 1
    Call FillGrid
End If
If bytMode <> 5 Then
...
Select Case bytMode
    Case 2          '' View Mode
        ...
        If MsgBox(NewCaptionTxt("00015", adrsMod), vbYesNo + vbQuestion) = vbYes Then
            ...
            cboLeave.Text & "'" & " and Trcd = 1" & " and FromDate=" & _
            strDTEnc & DateCompStr(txtFrom.Text) & strDTEnc
            Call AddActivityLog(lgDelete_Action, 3, 18)     '' Delete Log
            Call AuditInfo("DELETE", Me.Caption, "Delete Leave Opening Entry " & cboLeave.Text & " For Employee " & cboCode.Text)
...
On Error GoTo ERR_P                     '' Once the Opening  Balance Leave is Deleted
...
''  to solve errors about cancel and delete
...
Private Function ValidateAddmaster() As Boolean     '' Validate Details before Adding the
On Error GoTo ERR_P                                 '' Opening Balance Leave
ValidateAddmaster = True
'' Check if Any Leave is Selected or Not
If cboLeave.Text = "" Then
    MsgBox NewCaptionTxt("37003", adrsC), vbExclamation
    cboLeave.SetFocus
    ValidateAddmaster = False
    Exit Function
End If
'' Check for Invalid Number of Opening Balance Leave Days
If Val(txtDays.Text) <= 0 Then
    MsgBox NewCaptionTxt("37004", adrsC), vbExclamation
    txtDays.SetFocus
    ValidateAddmaster = False
    Exit Function
End If
...
If Right(txtDays.Text, 2) <> "00" And Right(txtDays.Text, 2) <> "50" Then
    MsgBox NewCaptionTxt("37005", adrsC)
    txtDays.SetFocus
    ValidateAddmaster = False
    Exit Function
End If
...
'' Check if Leaves are Already Added as Opening Balance for the Specified Dates
...
DateDiff("m", Year_Start, DateCompDate(txtFrom.Text)) < 0 Then
    MsgBox NewCaptionTxt("00030", adrsMod) & txtFrom.Text & NewCaptionTxt("00021", adrsMod), vbExclamation
    txtFrom.SetFocus
    ValidLeaveDate = False
    Exit Function
End If
...
Private Function ALreadyAvailedDate() As Boolean    '' Checks if Leave is Already Added as
On Error GoTo ERR_P                                 '' Opening Balance by the Employee
ALreadyAvailedDate = True                           '' for the Same Dates
...
strDTEnc & " and Trcd= 1" & " and Empcode=" & "'" & cboCode.Text & "'" & " and Lcode=" & _
"'" & cboLeave.Text & "'"
If adrsPaid.State = 1 Then adrsPaid.Close
adrsPaid.Open strA_R, ConMain
If Not (adrsPaid.EOF And adrsPaid.BOF) Then
        MsgBox NewCaptionTxt("37006", adrsC), vbExclamation
        txtFrom.SetFocus
...
"(" & "'" & cboCode.Text & "'" & "," & "1" & "," & strDTEnc & DateSaveIns(txtFrom.Text) & _
...
On Error GoTo ERR_P
Call AddActivityLog(lgADD_MODE, 3, 18)     '' Add Activity
Call AuditInfo("ADD", Me.Caption, "Add Leave Opening Entry " & cboLeave.Text & " For Employee " & cboCode.Text)
Exit Sub
ERR_P:
