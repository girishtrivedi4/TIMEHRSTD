...
Begin VB.Form frmCredit 
   ...
   Begin TabDlg.SSTab TB1 
      ...
      EndProperty
      TabCaption(0)   =   "List"
      TabPicture(0)   =   "frmCredit.frx":0000
      Tab(0).ControlEnabled=   -1  'True
      Tab(0).Control(0)=   "MSF1"
      Tab(0).Control(0).Enabled=   0   'False
      Tab(0).ControlCount=   1
      TabCaption(1)   =   " "
      TabPicture(1)   =   "frmCredit.frx":001C
      Tab(1).ControlEnabled=   0   'False
      Tab(1).Control(0)=   "frCredit"
      Tab(1).Control(0).Enabled=   0   'False
      Tab(1).ControlCount=   1
      Begin VB.Frame frCredit 
         Height          =   2805
         Left            =   -74940
         TabIndex        =   12
         Top             =   360
...
adrsC.Open "Select * from NewCaptions Where ID Like '07%' or ID Like '15%'", _
ConMain, adOpenStatic
Me.Caption = NewCaptionTxt("15001", adrsC)              '' Form Caption
...
lblFrom.Caption = NewCaptionTxt("15002", adrsC)         '' From
...
adrsPaid.Open "Select Cat from EmpMst where EmpCode='" & cboCode.Text & "'" _
, ConMain
...
Private Sub FillGrid()      '' Fills the Grid with the Leaves the Employee Has Credited
On Error GoTo ERR_P
Dim bytCnt As Byte
If adrsDept1.State = 1 Then adrsDept1.Close
adrsDept1.Open "Select Lcode,FromDate,ToDate,Days  From " & "LvInfo" & _
Right(pVStar.YearSel, 2) & " where Trcd=2" & " and Empcode=" & "'" & _
...
If Not (adrsDept1.EOF And adrsDept1.BOF) Then
    For intTmp = 0 To adrsDept1.Fields.Count - 1
        If UCase(adrsDept1(intTmp).name) <> "EMPCODE" Then  '' if Field is not Employee Code
            ...
            If Not (adrsRits.EOF And adrsRits.BOF) Then         '' if Leave Desc is Found
                ...
                bytTmp = bytTmp + 1
                If SubLeaveFlag = 1 And (UCase(adrsDept1(intTmp).name) = "HP" Or UCase(adrsDept1(intTmp).name) = "CM" Or UCase(adrsDept1(intTmp).name) = "EL") Then bytTmp = bytTmp - 1   ' 07-11
            End If
        End If
    Next
    ...
    For intTmp = 0 To adrsDept1.Fields.Count - 1
        If UCase(adrsDept1(intTmp).name) <> "EMPCODE" Then  '' if Field is not Employee Code
            If Not (SubLeaveFlag = 1 And (UCase(adrsDept1(intTmp).name) = "HP" Or UCase(adrsDept1(intTmp).name) = "CM" Or UCase(adrsDept1(intTmp).name) = "EL")) Then   ' 15-10
                If adrsRits.State = 1 Then adrsRits.Close
                adrsRits.Open "Select Leave from LeavDesc where LvCode='" & _
                adrsDept1(intTmp).name & "' and cat='" & strCatAvail & "'", ConMain
                If Not (adrsRits.EOF And adrsRits.BOF) Then         '' if Leave Desc is Found
...
Call GetCat
If bytMode <> 4 Then
    '' Get All the Leaves the Employee has Credited that Year
    bytMode = 1
    Call FillGrid
End If
If bytMode <> 5 Then
...
Select Case bytMode
    Case 2          '' View Mode
        ...
        If MsgBox(NewCaptionTxt("00015", adrsMod), vbQuestion + vbYesNo) = vbYes Then
            ...
            cboLeave.Text & "'" & " and Trcd = 2" & " and FromDate=" & _
            strDTEnc & DateCompStr(txtFrom.Text) & strDTEnc
            Call AddActivityLog(lgDelete_Action, 3, 19)     '' Delete Log
            Call AuditInfo("DELETE", Me.Caption, "Delete Leave Credit Entry " & cboLeave.Text & " For Employee " & cboCode.Text)
...
On Error GoTo ERR_P                     '' Once the Credited Leave is Deleted
...
If SubLeaveFlag = 1 And (cboLeave.Text = "EN" Or cboLeave.Text = "NE") Then ' 15-10
...
Private Function ValidateAddmaster() As Boolean     '' Validate Details befor Crediting the
On Error GoTo ERR_P                                 '' Leave
ValidateAddmaster = True
'' Check if Any Leave is Selected or Not
If cboLeave.Text = "" Then
    MsgBox NewCaptionTxt("15003", adrsC), vbExclamation
    cboLeave.SetFocus
    ValidateAddmaster = False
    Exit Function
End If
'' Check for Invalid Number of CreditLeave Days
If Val(txtDays.Text) <= 0 Then
    MsgBox NewCaptionTxt("15004", adrsC), vbExclamation
    txtDays.SetFocus
    ValidateAddmaster = False
    Exit Function
End If
txtDays.Text = Format(txtDays.Text, "0.00")
    ...
    If Right(txtDays.Text, 2) <> "00" And Right(txtDays.Text, 2) <> "50" Then
        MsgBox NewCaptionTxt("15005", adrsC), vbExclamation
        txtDays.SetFocus
        ValidateAddmaster = False
        Exit Function
    End If
...
If Not ValidLeaveDate Then
    ...
    Exit Function
End If
' ' Check if the days > maximum Credit Limit
If Not MaxLimit Then
    ValidateAddmaster = False
    Exit Function
End If
Call NumOfBalance           '' Procedure to Get Number of Leaves Balance
'' Check if Leaves are Already Credited for the Specified Dates
...
If txtFrom.Text = "" Then
    MsgBox NewCaptionTxt("15006", adrsC), vbExclamation
    txtFrom.SetFocus
...
Private Function ALreadyAvailedDate() As Boolean    '' Checks if Leave is Already Credited
...
strDTEnc & " and Trcd= 2" & " and Empcode=" & "'" & cboCode.Text & "'" & " and Lcode=" & _
"'" & cboLeave.Text & "'"
If adrsPaid.State = 1 Then adrsPaid.Close
adrsPaid.Open strA_R, ConMain
If Not (adrsPaid.EOF And adrsPaid.BOF) Then
        MsgBox NewCaptionTxt("15007", adrsC), vbExclamation
        txtFrom.SetFocus
...
If cboLeave.Text = "CO" Then
    EntryDate = DateSaveIns(txtFrom.Text)
Else
    EntryDate = Date
End If
...
"(" & "'" & cboCode.Text & "'" & "," & "2" & "," & strDTEnc & DateSaveIns(txtFrom.Text) & _
strDTEnc & "," & strDTEnc & DateSaveIns(txtFrom.Text) & strDTEnc & "," & "'" & _
cboLeave.Text & "'" & "," & txtDays.Text & "," & "'" & strRW & _
"'" & "," & "'" & DateSaveIns(CStr(EntryDate)) & "'" & ")"
...
Private Function MaxLimit() As Boolean
On Error GoTo ERR_P
MaxLimit = True
If adrsPaid.State = 1 Then adrsPaid.Close
adrsPaid.Open "Select Lv_Qty from leavdesc where LvCode=" & "'" & cboLeave.Text & "'" _
, ConMain, adOpenKeyset, adLockOptimistic
If Val(txtDays.Text) > adrsPaid("Lv_Qty") Then
    If MsgBox(NewCaptionTxt("15008", adrsC) & adrsPaid("Lv_Qty") & NewCaptionTxt("15009", adrsC) & vbCrLf & _
    NewCaptionTxt("15010", adrsC), vbYesNo + vbQuestion, App.EXEName) = vbYes Then
        MaxLimit = True
    Else
        MaxLimit = False
    End If
...
Exit Function
ERR_P:
    ShowError ("MaxLimit :: " & Me.Caption)
    MaxLimit = False
End Function

Private Sub SaveAddLog()            '' Procedure to Save the Add Log
On Error GoTo ERR_P
Call AddActivityLog(lgADD_MODE, 3, 19)     '' Add Activity
Call AuditInfo("ADD", Me.Caption, "Added Leave Credit Entry " & cboLeave.Text & " For Employee " & cboCode.Text)
Exit Sub
ERR_P:
